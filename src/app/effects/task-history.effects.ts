...
export class TaskHistoryEffects {
  ...
  loadTaskHistories$: Observable<Action> = this.actions$
    .pipe(
      ...
      switchMap((taskId: string) => this.services$.getTaskHistory(taskId)
        .pipe(
          map((taskHistories: TaskHistory[]) => new actions.LoadHistorySuccessAction(taskHistories)),
          catchError(err => of(new actions.LoadHistoryFailAction(JSON.stringify(err))))
        )
      )
    );
      ...
      mergeMap(([data, user]: [{ taskId: string; operation: History.TaskOperations }, User]) => {
        ...
        return this.services$.addTaskHistory(taskHistory)
          .pipe(
            ...
            catchError(err => of(new actions.AddTaskHistoryFailAction(JSON.stringify(err))))
          );
      })
    );
  ...
  addUpdateTaskHistory$: Observable<{ selectedTaskVM: TaskVM; updatedTaskVM: TaskVM; user: User }> = this.actions$
    .pipe(
      ...
      map(action => action.payload),
      withLatestFrom(this.store$.pipe(select(fromRoot.getSelectedTask)), (t: Task, selectedTaskVM: TaskVM) => selectedTaskVM),
      withLatestFrom(
        this.store$.pipe(select(fromRoot.getUpdatedTask)),
        (selectedTaskVM: TaskVM, updatedTaskVM: TaskVM) => ({ selectedTaskVM: selectedTaskVM, updatedTaskVM: updatedTaskVM })),
      withLatestFrom(this.store$.pipe(select(fromRoot.getAuthUser)), (val, user: User) => ({ ...val, user: user })),
      tap((data: { selectedTaskVM: TaskVM; updatedTaskVM: TaskVM; user: User }) => {
