...
export class TaskFilterEffects {
  ...
  loadTaskFilter$: Observable<Action> = this.actions$
    .pipe(
      ...
      switchMap((taskFilterId: string) => this.service$.getTaskFilter(taskFilterId)
        .pipe(
          map((taskFilter: TaskFilter) => new actions.LoadTaskFilterSuccessAction(taskFilter)),
          catchError(err => of(new actions.LoadTaskFilterFailAction(JSON.stringify(err))))
        )
      )
    );
  ...
  addTaskFilter$: Observable<Action> = this.actions$
    .pipe(
      ...
      map(action => action.payload),
      switchMap((project: Project) => this.service$.addTaskFilter(getToAddTaskFilter(<string>project.id))
        .pipe(
          map((taskFilter: TaskFilter) => new actions.AddTaskFilterSuccessAction({ ...project, taskFilterId: taskFilter.id })),
          catchError(err => of(new actions.AddTaskFilterFailAction(JSON.stringify(err))))
        )
      )
    );
    ...
    .pipe(
      ...
      mergeMap(([taskFilterVM, taskFilter]: [TaskFilterVM, TaskFilter]) => {
        return this.service$.updateTaskFilter(getToUpdateTaskFilter(taskFilter, taskFilterVM))
          .pipe(
            map((tf: TaskFilter) => new actions.UpdateTaskFilterSuccessAction(tf)),
            catchError(err => of(new actions.UpdateTaskFilterFailAction(JSON.stringify(err))))
          );
      })
    );
