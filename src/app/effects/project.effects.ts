...
import * as projectTaskHistoryActions from '../actions/project-task-history.action';
...
export class ProjectEffects {
  ...
  loadProjectHistory$: Observable<Action> = this.actions$
    .ofType<actions.SelectProjectAction>(actions.SELECT)
    .map(action => action.payload)
    .map(project => new projectTaskHistoryActions.LoadProjectHistoryAction(<string>project.id));
