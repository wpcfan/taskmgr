...
import { observeQuery } from './query-observable';
import { 
  Query, 
...
export function WilddogListFactory (
  ref: DatabaseReference,
  { preserveSnapshot, query = {} } :WilddogListFactoryOpts = {}): WilddogListObservable<any> {

  if (utils.isEmptyObject(query)) {
    return wilddogListObservable(ref, { preserveSnapshot });
  }

  const queryObs = observeQuery(query);
  return new WilddogListObservable(ref, subscriber => {
    let sub = switchMap.call(map.call(queryObs, query => {
      ...
      return queried;
    }), (queryRef: wilddog.sync.Reference, ix: number) => {
      return wilddogListObservable(queryRef, { preserveSnapshot });
    })
 ...
 */
function wilddogListObservable(ref: wilddog.sync.Reference | DatabaseQuery, {preserveSnapshot}: WilddogListFactoryOpts = {}): WilddogListObservable<any> {
  ...
  const listObs = new WilddogListObservable(ref, (obs: Observer<any[]>) => {
...
export function onChildAdded(arr:any[], child:any, toKey:(element:any)=>string, prevKey:string): any[] {
  ...
  return arr.reduce((accumulator:wilddog.sync.DataSnapshot[], curr:wilddog.sync.DataSnapshot, i:number) => {
    if (!prevKey && i===0) {
